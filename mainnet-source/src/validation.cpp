...
static constexpr int PARALLAX_FORK_HEIGHT = 50000;
unsigned int GetParallaxForkWorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params);
    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // Parallax: Only support BIP125 RBF when -mempoolreplacement arg is set
...
bool ReadRawBlockFromDisk(std::vector<uint8_t>& block, const FlatFilePos& pos, const CMessageHeader::MessageStartChars& message_start)
{
    ...
    try {
        ...
        const CMessageHeader::MessageStartChars legacy_disk_start = {0xfd, 0xd2, 0xc8, 0xf1};
        const CMessageHeader::MessageStartChars para_disk_start = {0x50, 0x41, 0x52, 0x41};

        const bool matches_current_disk_start =
            memcmp(blk_start, message_start, CMessageHeader::MESSAGE_START_SIZE) == 0;

        const bool matches_legacy_disk_start =
            memcmp(blk_start, legacy_disk_start, CMessageHeader::MESSAGE_START_SIZE) == 0;

        const bool matches_para_disk_start =
            memcmp(blk_start, para_disk_start, CMessageHeader::MESSAGE_START_SIZE) == 0;

        if (!matches_current_disk_start && !matches_legacy_disk_start && !matches_para_disk_start) {
            return error("%s: Block magic mismatch for %s: %s versus expected %s, legacy %s, or para %s", __func__, pos.ToString(),
                    HexStr(blk_start),
                    HexStr(message_start),
                    HexStr(legacy_disk_start),
                    HexStr(para_disk_start));
        }
    ...
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == ThresholdState::ACTIVE || state == ThresholdState::LOCKED_IN) {
                const bilingual_str warning = Untranslated("Welcome to Parallax (PLX). Have fun mining.");
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    const unsigned int nExpectedBits = GetNextWorkRequired(pindexPrev, &block, consensusParams);
    if (block.nBits != nExpectedBits) {
    ...
    if (nHeight >= 4763002) {
        for (const CTxOut& txout : block.vtx[0]->vout) {
            ...
            if (whichType != TxoutType::WITNESS_V0_KEYHASH) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-parallax-payout", "coinbase payout must be witness_v0_keyhash after Parallax activation");
            }
        }
    }
