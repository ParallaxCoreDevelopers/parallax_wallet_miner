...
private:
    ...
    QPushButton* miningToggleButton = nullptr;
    QLabel* miningProgressLabel = nullptr;
    QTimer* miningProgressTimer = nullptr;
    bool m_miningActive{false};
    QString m_miningAddress;
    std::atomic_bool m_miningStopRequested{true};
    std::vector<std::thread> m_miningWorkers;
    QElapsedTimer m_miningRoundTimer;
    double m_miningCounterBase{0.0};
    double m_miningDisplayedValue{0.0};
    double m_miningHashrateEstimate{50000.0};
    double m_miningExpectedHashes{4294967296.0};
    ...
    void updateMiningProgressLabel(double value);
    void beginMiningRound(double difficulty);
    void loadMiningCounterBase();
    void persistMiningCounterBase() const;
    void resetMiningProgressLabel();
#ifdef ENABLE_WALLET
    bool ensureMiningAddress(QString& address);
    void stopMiningWorkers();
    void miningWorker(QString mining_address);
#endif
