...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    if (plx_input_amount > 0) {
        const CAmount plx_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(plx_fee <= plx_input_amount);
        pegin_amount = (plx_input_amount - (plx_fee + plx_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, plx_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& plx_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + plx_change);
    if (change_amount < 0) {
